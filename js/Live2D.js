...
$(document).mousemove(function(e){
	...
	if(l2d.length!=0){
		if($("#l2dTipBar").length==0){
			...
			l2dFade(l2d, (localStorage.l2dShow&&localStorage.l2dShow==1));
			setInterval(checkTip,100);
		}
		if((checkCursorl2d(e.pageX, e.pageY, l2d.offset()))&&(localStorage.l2dShow&&localStorage.l2dShow==1)&&l2dlv){
			msgTip(MsgList[Math.floor(Math.random()*MsgList.length)],0);
			l2dlv=false;
		}else if(!(checkCursorl2d(e.pageX, e.pageY, l2d.offset()))&&!l2dlv){
			l2dlv=true;
		}
	}
});
...
$(document).click(function(e){
	...
	if(l2d.length!=0){
		if(checkCursorl2d(e.pageX, e.pageY, l2d.offset())){
			if(!localStorage.l2dShow||localStorage.l2dShow==0){
...
function checkTip(){
	...
	if((!localStorage.l2dShow||localStorage.l2dShow==0)){
		if(TipTimer<30&&(l2dTip[0].style.opacity==0)){
			l2dFade($('#live2d-widget'), true);
		}
		if(TipTimer>=30&&(l2dTip[0].style.opacity==0.8)){
			l2dFade($('#live2d-widget'), false);
		}
	}
	if(TipTimer<30){
